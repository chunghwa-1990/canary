...
import com.example.canary.exception.BusinessException;
...
public class GlobalExceptionHandler {
    ...
    public ResultEntity<?> httpRequestMethodNotSupportedExceptionHandler(HttpRequestMethodNotSupportedException ex) {
        ...
        // message key
        String messageKey = ResultCodeEnum.Common.METHOD_NOT_ALLOWED.getMessage();
        // 根据 message key 查询 i18n 实现国际化
        String message = messageSource.getMessage(messageKey, args.toArray(), ex.getMessage(), LocaleContextHolder.getLocale());
        return ResultEntity.fail(ResultCodeEnum.Common.METHOD_NOT_ALLOWED.getCode(), message);
    }
    ...
    public ResultEntity<?> missingServletRequestParameterExceptionHandler(MissingServletRequestParameterException ex) {
        ...
        // message key
        String messageKey = ResultCodeEnum.Common.BAD_REQUEST.getMessage();
        // 根据 message key 查询 i18n 实现国际化
        String message = messageSource.getMessage(messageKey, args.toArray(), ex.getMessage(), LocaleContextHolder.getLocale());
        return ResultEntity.fail(ResultCodeEnum.Common.BAD_REQUEST.getCode(), message);
    }
    ...
    public ResultEntity<?> missingServletRequestPartExceptionHandler(MissingServletRequestPartException ex) {
        ...
        // message key
        String messageKey = ResultCodeEnum.Common.BAD_PART_REQUEST.getMessage();
        // 根据 message key 查询 i18n 实现国际化
        String message = messageSource.getMessage(messageKey, args.toArray(), ex.getMessage(), LocaleContextHolder.getLocale());
        return ResultEntity.fail(ResultCodeEnum.Common.BAD_REQUEST.getCode(), message);
    }
    ...
    public ResultEntity<?> methodArgumentNotValidHandler(MethodArgumentNotValidException ex) {
        ...
        if (fieldError == null) {
            return ResultEntity.fail(ResultCodeEnum.Common.BAD_REQUEST.getCode(), ex.getMessage());
        }
        // 字段名称
    ...
    public ResultEntity<?> RuntimeExceptionHandler(RuntimeException ex) {
        if (StringUtils.hasText(ex.getMessage())) {
            return ResultEntity.fail(ResultCodeEnum.Common.ERROR.getCode(), ex.getMessage());
        }
        return ResultEntity.fail(ResultCodeEnum.Common.ERROR);
    }
}
